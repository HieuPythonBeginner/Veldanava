...
private:
    ...
    Token scan_string(char quote) {
        ...
        TokenType type = (quote == '\'') ? TokenType::CharLit : TokenType::StringLit;
    ...
    Token scan_directive() {
        ...
        std::string directive = src_.substr(start, pos_ - start);
        if (directive == "#Pre_Descent") {
            return Token(TokenType::Pre_Descent, directive, line_, col_);
        }
        if (directive == "#Beginning_Eden") {
            skip_line_comment();
            return Token(TokenType::Beginning_Eden, directive, line_, col_);
        }
        skip_line_comment();
        return Token(TokenType::Error, directive);
    }
};
